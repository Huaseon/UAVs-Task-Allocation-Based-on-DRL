...
save_dir = Path("output") / "ablation" / f"{max_times}-{exploration_rate_min:.4f}" / "001"
save_dir.mkdir(parents=True) if not save_dir.exists() else None

variant001_agent = Agent(save_dir=save_dir, variant="001", max_times=max_times, exploration_rate_min=exploration_rate_min)
logger = AgentLogger(save_dir=save_dir, name="001", top=top)

for episode in range(top):
    drone_state, task_state, action_state = env.reset()

    while True:
        action = variant001_agent.act(drone_state=drone_state, task_state=task_state, action_state=action_state)

        next_drone_state, next_task_state, next_action_state, reward, done = env.step(action=action)

        variant001_agent.cache(drone_state=drone_state, task_state=task_state, action_state=action_state, next_drone_state=next_drone_state, next_task_state=next_task_state, action=action, reward=reward, done=done)
        
        q, loss = variant001_agent.learn()
    ...
    logger.record(episode=episode, epsilon=variant001_agent.exploration_rate, step=variant001_agent.curr_step)
    logger.wind.save([logger.name])

variant001_agent.save()
logger.wind.save([logger.name])