...
class TestLogger:
    def __init__(self, save_dir, top, name):
        ...
        self.wind.win_exists('r_plot') or self.wind.line(
                [0], [0], win='r_plot', opts=dict(title='r_plot', legend=['precision_realtime']), name=f'precision_realtime', update='append'
        )
    ...
    def record(self, episode):
        ...
        for y, m, name_part in zip(self.ep_avg_rs[-1], mean_ep_r, ['precision', 'payload', 'temporal', 'optimization']):
            self.wind.line(
                [y], [episode], win='r_plot', name=f'{name_part}_realtime', update='append',
            )
            self.wind.line(
