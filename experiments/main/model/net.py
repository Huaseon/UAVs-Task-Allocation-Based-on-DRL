...
class PolicyNet(nn.Module):
    def __init__(self, is_base: bool) -> None:
        super().__init__()
        
        self.online = _build_net(is_base=is_base)
        self.target = _build_net(is_base=is_base)
...
class _build_net(nn.Module):
    def __init__(self, is_base: bool) -> None:
        ...
        self.drone_encoder: nn.Sequential = nn.Sequential(
            ...
            nn.ReLU(),
            self.shared_module if not is_base else nn.Linear(in_features=DRONE_FEATURES_SIZE * TASK_FEATURES_SIZE, out_features=DRONE_FEATURES_SIZE * TASK_FEATURES_SIZE),
            nn.ReLU(),
        ...
        self.task_encoder: nn.Sequential =  nn.Sequential(
            ...
            nn.ReLU(),
            self.shared_module if not is_base else nn.Linear(in_features=TASK_FEATURES_SIZE * DRONE_FEATURES_SIZE, out_features=DRONE_FEATURES_SIZE * TASK_FEATURES_SIZE),
            nn.ReLU(),
            nn.Linear(in_features=DRONE_FEATURES_SIZE * TASK_FEATURES_SIZE, out_features=DRONE_FEATURES_SIZE * TASK_FEATURES_SIZE),
            nn.ReLU(),
        )
        self.Q_net: _q_net = _q_net(is_base=is_base)
        self.updater: _gate_updater = _gate_updater(is_base=is_base)
