...
# %%
class Agent:
    def __init__(self, save_dir: Path, lr: float, sync_every_rate: float, gamma: float, max_times: float=4, exploration_rate_min: float=3/4, is_base: bool=False) -> None:
        ...
        self.sync_every: int = np.rint(max_size * self.max_times * sync_every_rate).astype(int) # 参数同步周期

        self.optimizer: torch.optim.Adam = torch.optim.Adam(self.net.parameters(), lr=lr) # 使用Adam优化算法

        self.loss_fn: nn.SmoothL1Loss = nn.SmoothL1Loss() # 使用Smooth L1 Loss损失函数

        self.gamma = gamma # 未来奖励贴现
    ...
    def loads(self):
        ...
        return self.load(save_model=save_model)

    def load(self, save_model):
        self.net.load_state_dict(save_model['model'])
        self.is_base = save_model['is_base']
