...
from_path = Path("output") / "ablation" / f"{max_times}-{exploration_rate_min:.4f}" / "100"
to_path = Path("output") / "test" / f"{max_times}-{exploration_rate_min:.4f}" / "100"
...
top = 1000
logger = TestLogger(save_dir=to_path, top=top, name="test-100")
