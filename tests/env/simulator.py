...
class Environment:
    ...
    def step(self, action: np.ndarray) -> tuple[np.ndarray, np.ndarray, np.ndarray, float, bool, np.ndarray]:
        """
        `action`：作出的决策
        决策更新 -> (状态更新，奖励)
        ...
        reward_delta, self.cur_reward = reward[-1] - self.cur_reward, reward[-1]
        return *self.obs(), reward_delta, self.action_space.done(), reward[0]
