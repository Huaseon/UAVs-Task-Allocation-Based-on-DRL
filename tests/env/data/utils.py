...
# %%
def step(drone: Drone, tasks: np.ndarray[Task]) -> tuple[float, float, float, float]:
    beta = tasks.size
    if beta == 0: return 1, 1, 1, 0, 0

    tasks = legal_tasks(drone=drone, tasks=tasks)
    
    beta = tasks.size / beta
    if beta == 0: return 0, 1, 1, 0, 0
    ...
    return beta, pyl_limit, tpr_limit, opm_reward, beta * pyl_limit * tpr_limit * opm_reward
