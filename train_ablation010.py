...
save_dir = Path("output") / "ablation" / f"{max_times}-{exploration_rate_min:.4f}" / "010"
save_dir.mkdir(parents=True) if not save_dir.exists() else None

variant010_agent = Agent(save_dir=save_dir, variant="010", max_times=max_times, exploration_rate_min=exploration_rate_min)
logger = AgentLogger(save_dir=save_dir, name="010", top=top)

for episode in range(top):
    drone_state, task_state, action_state = env.reset()

    while True:
        action = variant010_agent.act(drone_state=drone_state, task_state=task_state, action_state=action_state)

        next_drone_state, next_task_state, next_action_state, reward, done = env.step(action=action)

        variant010_agent.cache(drone_state=drone_state, task_state=task_state, action_state=action_state, next_drone_state=next_drone_state, next_task_state=next_task_state, action=action, reward=reward, done=done)
        
        q, loss = variant010_agent.learn()
    ...
    logger.record(episode=episode, epsilon=variant010_agent.exploration_rate, step=variant010_agent.curr_step)
    logger.wind.save([logger.name])

variant010_agent.save()
logger.wind.save([logger.name])